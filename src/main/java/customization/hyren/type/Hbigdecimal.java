...
public class Hbigdecimal implements ByteArrayValueMapper {

    private static Log log = LogFactory.getLog(Hbigdecimal.class);
    /**
        ...
        try {
            ...
            log.warn(
                    String.format("Error mapping byte value %s to %s", Bytes.toStringBinary(input),
                            BigDecimal.class.getName()), e);
            return ImmutableList.of();
        }
    }

    private BigDecimal mapInternal(String toString) {
        return BigDecimal.valueOf(Long.parseLong(toString));
    }
}
