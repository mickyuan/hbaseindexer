...
public class Hfloat implements ByteArrayValueMapper {

    private static Log log = LogFactory.getLog(Hfloat.class);
    /**
        ...
        try {
            ...
            log.warn(
                    String.format("Error mapping byte value %s to %s", Bytes.toStringBinary(input),
                            float.class.getName()), e);
            return ImmutableList.of();
        }
    }

    private float mapInternal(String toString) {
