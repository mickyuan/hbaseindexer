...
public class Hstring implements ByteArrayValueMapper {

    private static Log log = LogFactory.getLog(Hstring.class);
    ...
    public Collection<? extends Object> map(byte[] input) {
        try {
            ...
            log.warn(
                    String.format("Error mapping byte value %s to %s", Bytes.toStringBinary(input), String.class.getName()), e);
            return ImmutableList.of();
        }
    }

    private String mapInternal(String toString) {
        return toString;
    }
}
